...
- **Lockfile / CI gate**: the tox gate installs dependencies from the committed `uv.lock` via tox-uv's `uv-venv-lock-runner` (CI reproducible, no drift when upstream releases cross the `exclude-newer` window). Dependency upgrades happen as deliberate `uv lock` refreshes committed as reviewable lockfile diffs; `exclude-newer` applies at lock-refresh time, not per CI run. The `installed`, `security`, and `nopyarrow` tox envs intentionally still resolve fresh (sdist/published-package test paths, no-pyarrow resolution).

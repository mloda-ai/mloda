...
- **Lockfile / CI gate**: the default tox env installs dependencies from the committed `uv.lock` via tox-uv's `uv-venv-lock-runner`, so CI is reproducible and does not drift when upstream releases cross the `exclude-newer` window. Dependency upgrades happen as deliberate `uv lock` refreshes committed as reviewable lockfile diffs, so `exclude-newer` applies when the lock is refreshed, not per CI run. The `installed`, `security`, and `nopyarrow` tox envs intentionally still resolve fresh, since they test the sdist/published-package paths or a no-pyarrow resolution.

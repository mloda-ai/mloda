...
An extender holding a runtime handle that pickle can't round-trip (a client, tracer, connection, lock) must rebuild it lazily, e.g. inside `__call__` or `__setstate__`, instead of setting it eagerly in `__init__`.

...
`FEATURE_GROUP_MATCHED`, `INPUT_DATA_LOAD`, and `JOIN` populate their own extra fields and leave the rest at their defaults. On `INPUT_DATA_LOAD`, `data_access_identity` and `data_access_format` carry a string identity and format for the data-access handle/value: the reader's `data_access_identity(data_access)` classmethod decides it. The default, in order: sorted key names for a mapping; scheme/host/port/path for a string that fullmatches `scheme://host[:port]/path`, user info, query and fragment dropped, an `abfs`/`abfss`/`wasb`/`wasbs` container kept, the path cut back to the last `/` before its first percent-escape and dropped entirely for a `jdbc:` scheme, and the type name instead when any other scheme's path has a segment with a `:` before a `=`; an existing local path (`os.path.exists`), as given; else the type name. The kept URI or local path is published as given, `key=value` segments included, so a reader whose paths carry tokens should override this method. An override must not raise: it runs only when an `INPUT_DATA_LOAD` extender is registered, and its exception aborts the load. Extenders must record this field, never the raw `data_access` argument, which may carry credentials. `data_access_dataset_version` stays `None`, since no dataset versioning exists yet. On `JOIN`, `join_type` and `join_keys` come from the `Link` being merged. On `FEATURE_GROUP_MATCHED`, `plan_feature_count`, `plan_node_count`, and `plan_depth` are running counts and recursion depth at match time, not final totals for the whole plan.

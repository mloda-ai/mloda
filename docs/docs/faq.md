...
mloda is designed for AI agents, ML pipelines, data scientists, and data practitioners who need declarative data access. It helps implement repeatable feature creation and apply best practices borrowed from fields like software engineering, data modeling, and testing.
...
#### How can AI agents use mloda?

AI agents can request data using JSON configuration without writing Python code. Use `load_features_from_config` to parse JSON feature requests generated by LLMs:
...
# An LLM emits this JSON; it writes no Python itself.
llm_json_output = """
[
    "customer_id",
...
features = load_features_from_config(llm_json_output, format="json")
...
#### Can LLMs generate mloda plugins?

Yes. Plugins are small, template-like structures that are easy for AI to generate. The declarative nature of feature groups makes them ideal for AI-assisted development.

...
- **input_feature_edges** (`Mapping[str, tuple[str, ...]]`): Each output feature name of a compute step mapped to its sorted declared input names (injected features absent, empty for a root step). Prepare-time twin of `HookContext.input_feature_edges`.

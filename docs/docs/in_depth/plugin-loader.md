...
- **Skips are visible.** An in-tree `mloda_plugins` module skipped for a missing optional dependency logs one WARNING naming the module and the dependency. `PluginLoader.skipped_plugins()` returns every skipped module and entry point with its missing dependency; `reset_cache()` clears it.

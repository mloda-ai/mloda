...
- **Missing optional dependencies skip.** `ImportError` (covers `ModuleNotFoundError` too) around an entry point's `.load()` is caught. Its root module is checked against that entry point's `mloda.optional_dependencies` declaration, or else the global `OPTIONAL_PLUGIN_DEPENDENCIES` (pandas, polars, duckdb, ...). A match skips only that entry point and logs at WARNING naming the entry point and the missing module; otherwise the error re-raises.
- **Own-package failures always raise.** If the missing root is the entry point's own module root, it re-raises even if declared optional: the plugin's own package is broken, not a missing third-party dependency.

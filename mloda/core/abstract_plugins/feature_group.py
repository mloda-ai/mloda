...
class FeatureGroup(ABC):
    ...
    def version(cls) -> str:
        """
        Version: mloda version, module name, and a hash of the code this class runs (first-party MRO classes, reachable
        functions, classes and constants; third-party per version_third_party_mode()). Override version() to customize.
        """
        return BaseFeatureGroupVersion.version(cls)

    @classmethod
    def version_third_party_mode(cls) -> ThirdPartyVersionMode:
        """Whether version()'s hash reacts to reachable third-party dependency versions; INCLUDE by default."""

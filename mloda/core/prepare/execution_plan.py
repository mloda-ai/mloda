...
class ExecutionPlan:
    ...
    def run_link(
        ...
        swap_sides = self.swap_merge_sides_by_declared_side(
            ...
            declared_left_framework=link_fw[1],
    ...
    def swap_merge_sides_by_declared_side(
        destination_framework: type[ComputeFramework],
        source_framework: type[ComputeFramework],
        declared_left_framework: type[ComputeFramework],
        ...
        always one of the key's two framework positions, and the first position is the declared left side.
        Links keyed on one single framework make that tautological, so the trekker-flip fallback stays for
        that case (currently unreachable via the public API, since same-framework RIGHT joins are rejected
        upstream for equal/polymorphic feature groups; kept as a defensive default, not dead certainty)."""
        ...
        if holds_right and not holds_left:
            return True
        if destination_framework != source_framework:
            return destination_framework != declared_left_framework
        return fallback

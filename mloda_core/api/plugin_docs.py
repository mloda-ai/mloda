...
Example:
    ...
    from mloda_core.api.plugin_docs import get_feature_group_docs
...
from mloda_core.api.plugin_info import ComputeFrameworkInfo, ExtenderInfo, FeatureGroupInfo
...
def get_feature_group_docs(
    ...
    """
    all_feature_groups = get_all_subclasses(AbstractFeatureGroup)
    results = []
...
def get_extender_docs(
    ...
    """
    all_extenders = get_all_subclasses(WrapperFunctionExtender)
    results = []
    ...
    for ext_class in all_extenders:
        ...
        if ext_name in ("WrapperFunctionExtender", "_CompositeExtender"):
            continue

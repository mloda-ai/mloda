...
class SqliteRelation(SqlBaseRelation):
    ...
    def _full_outer_join(
        ...
        # Unmatched right rows have a NULL self (left) join key. Same-key case: use the
        # shared column; differing-key case: recover the left key from the quoted condition.
        if shared:
            null_indicator = f"{q_self_alias}.{quote_ident(next(iter(shared)))}"
        else:
            ...
            null_indicator = f"{q_self_alias}.{match.group(1)}"
        where_clause = f"WHERE {null_indicator} IS NULL"

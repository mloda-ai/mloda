...
| Diagnostics | rejection reason surfaced, recorded on first pass, no phantom reason for unrelated groups, `expected` naming a non-strict guard's rejection | `test_option_value_rejection_never_escapes.py` (identify carries reason), `test_property_mapping_enforced_validators.py::TestRejectedValueNamedInResolutionError`, render layer under `test_prepare/`, `test_first_pass_rejection_recording.py::TestExpectedGuardRejectionRecording` |

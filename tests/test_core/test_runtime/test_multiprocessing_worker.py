...
class TestWorkerSignatureHasNoConnectionFlag:
    """The needs_tfs_connection prologue is gone: a worker's cfw resolves its own connection lazily via ensure_connection()."""

    def test_signature_has_no_needs_tfs_connection_parameter(self) -> None:
        signature = inspect.signature(worker)

        assert "needs_tfs_connection" not in signature.parameters

    def test_signature_parameter_order(self) -> None:
        signature = inspect.signature(worker)

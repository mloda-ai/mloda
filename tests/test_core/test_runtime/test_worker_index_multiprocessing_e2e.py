...
def _run_two_independent_root_feature_groups_once(tmp_path: Path, flight_server: Any, iteration: int) -> None:
    """One pass of the scenario above, isolated so a stress test can repeat it in a loop.

    A worker computing {"worker_index_e2e_col_one": [1, 2, 3]} is fast enough that its process can
    fully exit before ExecutionOrchestrator._check_for_error ever polls its result queue, which is
    exactly the false-positive "orphaned step" race this loop is meant to catch (see
    ExecutionOrchestrator._check_for_error in mloda/core/runtime/run.py). iteration namespaces the
    marker files so each pass gets its own, since tmp_path is shared across the whole stress test.
    """
    output_one = tmp_path / f"worker_index_one_{iteration}.txt"
    output_two = tmp_path / f"worker_index_two_{iteration}.txt"
    extender_one = _WorkerIndexRecordingExtender(output_one, _WorkerIndexFeatureGroupOne)
    extender_two = _WorkerIndexRecordingExtender(output_two, _WorkerIndexFeatureGroupTwo)
    ...
    assert output_one.exists(), f"iteration {iteration}: extender_one never observed calculate_feature"
    assert output_two.exists(), f"iteration {iteration}: extender_two never observed calculate_feature"


@pytest.mark.timeout(90)
class TestWorkerIndexMultiprocessingRaceStress:
    """Regression guard for the false-positive "orphaned step" race in
    ExecutionOrchestrator._check_for_error: a worker whose process exits before its result queue
    is ever polled must not be reported as having lost its result.

    A hard timing race like this is not deterministically reproducible in a single fast run --
    mocking the timing would just trade one flaky test for another. Repeating the real scenario
    several times in one test instead makes a regression of this exact race far more likely to be
    caught by a single CI run than a one-shot test would catch.
    """

    def test_repeated_runs_do_not_hit_the_orphaned_step_race(self, tmp_path: Path, flight_server: Any) -> None:
        for iteration in range(8):
            _run_two_independent_root_feature_groups_once(tmp_path, flight_server, iteration)

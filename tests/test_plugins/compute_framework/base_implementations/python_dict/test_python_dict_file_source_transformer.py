"""RED tests: the stdlib ``FileSource -> dict`` transformer must infer types COLUMN-WISE.
...
Fails today: ``_coerce`` runs PER CELL, so empty -> ``""``, ``"true"`` stays ``str``,
``"1_000"`` -> ``1000``, ``"NaN"`` -> ``float('nan')``; blank / short rows raise
``IndexError``; and a duplicated header silently resolves to its first occurrence.
"""

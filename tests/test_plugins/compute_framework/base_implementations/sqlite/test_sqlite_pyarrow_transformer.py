...
class TestSqlBasePyArrowTransformerImportError:
    def test_other_framework_raises_import_error_when_pyarrow_unavailable(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """other_framework() must raise ImportError when pyarrow is None, not return the class."""
        monkeypatch.setattr(
            "mloda_plugins.compute_framework.base_implementations.sql.sql_base_pyarrow_transformer.pa",
            None,
        )
        with pytest.raises(ImportError):
            SqlBasePyArrowTransformer.other_framework()

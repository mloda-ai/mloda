...
def test_integration_json_file() -> None:
    ...
    # Verify we got 12 features (including 2 multi-source features)
...
def test_end2end_nested_feature_references() -> None:
    ...
    # Second feature: scaled_age with mloda_source pointing to "age"
    assert isinstance(features[1], Feature)
    assert features[1].name.name == "scaled_age"
    assert features[1].options.context.get("mloda_source_feature") == "age"
    assert features[1].options.group.get("scaling_method") == "standard"

    # Third feature: derived_from_scaled with mloda_source pointing to scaled_age Feature object
    assert isinstance(features[2], Feature)
    assert features[2].name.name == "derived_from_scaled"
    # The mloda_source should be resolved to a Feature object, not a string
    mloda_source_2 = features[2].options.context.get("mloda_source_feature")
    assert isinstance(mloda_source_2, Feature)
    assert mloda_source_2.name.name == "scaled_age"
    assert features[2].options.group.get("transformation") == "log"

    # Fourth feature: nested_reference with mloda_source pointing to derived_from_scaled Feature object
    assert isinstance(features[3], Feature)
    assert features[3].name.name == "nested_reference"
    # The mloda_source should be resolved to a Feature object
    mloda_source_3 = features[3].options.context.get("mloda_source_feature")
    assert isinstance(mloda_source_3, Feature)
    assert mloda_source_3.name.name == "derived_from_scaled"
    ...
    chained_source_3 = mloda_source_3.options.context.get("mloda_source_feature")
    assert chained_source_3 is not None
    assert isinstance(chained_source_3, Feature)
    assert chained_source_3.name.name == "scaled_age"
...
def test_complete_integration_json() -> None:
    ...
    # Verify we got the expected number of features (12 total)
    ...
    # 3. Chained features (with __ in name and mloda_source)

...
def test_parse_json_with_mloda_source_field() -> None:
    """Test parsing JSON with mloda_source field for chained features.

    Chained features use the mloda_source field to specify the source feature
    that will be transformed. This test verifies that parse_json correctly
    handles and preserves the mloda_source field in FeatureConfig objects.
    """
    config_str = """[
        {
            "name": "scale__age",
    ...
    assert result[1].mloda_source == "weight"
...
def test_parse_chained_feature_string() -> None:
    ...
    features (objects with mloda_source field) in the same array.
    """
    config_str = """[
        "simple_feature",
        {
    ...
    assert result[1].mloda_source == "age"
    ...
    assert result[2].mloda_source is None
    ...
    assert result[3].mloda_source == "height"
...
def test_parse_json_handles_tilde_in_name() -> None:
    ...
    assert result[2].mloda_source == "source_col"
    assert result[2].column_index == 2


def test_parse_json_with_at_reference_in_mloda_source() -> None:
    """Test parsing JSON with @ reference syntax in mloda_source field.

    This test verifies that parse_json correctly handles @ prefix syntax
    in mloda_source fields. The @ prefix is used to reference other features
    ...
    # First: feature with @ reference in mloda_source
    assert isinstance(result[0], FeatureConfig)
    assert result[0].name == "scaled_feature"
    assert result[0].mloda_source == "@base_feature"
    assert result[0].options == {}

    # Second: feature with @ reference and options
    assert isinstance(result[1], FeatureConfig)
    assert result[1].name == "normalized_feature"
    assert result[1].mloda_source == "@scaled_feature"
...
def test_parse_json_preserves_reference_syntax() -> None:
    ...
    config_str = """[
        {
            "name": "feature1",
            "mloda_source": "@referenced_feature"
        },
        {
            "name": "feature2",
            "mloda_source": "regular_column"
        },
        {
            "name": "feature3",
            "mloda_source": "@another_reference",
    ...
    assert isinstance(result[0], FeatureConfig)
    assert result[0].name == "feature1"
    assert result[0].mloda_source == "@referenced_feature"
    assert result[0].mloda_source.startswith("@")

    # Second: regular column name without @
    assert isinstance(result[1], FeatureConfig)
    assert result[1].name == "feature2"
    assert result[1].mloda_source == "regular_column"
    assert not result[1].mloda_source.startswith("@")

    # Third: @ reference preserved with other fields
    assert isinstance(result[2], FeatureConfig)
    assert result[2].name == "feature3"
    assert result[2].mloda_source == "@another_reference"
    assert result[2].mloda_source.startswith("@")
...
def test_parse_json_with_mloda_sources_array() -> None:
    ...
    assert result[0].mloda_source is None
    assert result[0].options == {}
    ...
    assert result[1].mloda_source is None
...
def test_parse_json_rejects_mloda_source_and_mloda_sources_together() -> None:
    """Test that parse_json raises ValueError when both mloda_source and mloda_sources are provided.

    This test verifies that the mutual exclusion validation works during parsing,
    ensuring that features cannot use both mloda_source and mloda_sources fields
    simultaneously.
    """
    config_str = """[
        {
            "name": "invalid_feature",
            "mloda_source": "single_source",
            "mloda_sources": ["source1", "source2"]
        }
    ]"""

    with pytest.raises(ValueError, match="Cannot use both 'mloda_source' and 'mloda_sources'"):
        parse_json(config_str)

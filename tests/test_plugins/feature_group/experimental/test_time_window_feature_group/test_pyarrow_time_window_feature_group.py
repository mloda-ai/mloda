...
class TestPyArrowTimeWindowFeatureGroup:
    ...
    def test_calculate_feature_single(self, sample_time_table: pa.Table, feature_set_avg_window: FeatureSet) -> None:
        ...
        assert "avg_3_day_window_temperature" in result.schema.names
        ...
        assert result.column("avg_3_day_window_temperature")[0].as_py() == 20
        assert abs(result.column("avg_3_day_window_temperature")[1].as_py() - 21) < 0.1
        assert abs(result.column("avg_3_day_window_temperature")[2].as_py() - 20.33) < 0.1
    ...
    def test_calculate_feature_multiple(
        ...
        assert "avg_3_day_window_temperature" in result.schema.names
        assert "max_5_day_window_humidity" in result.schema.names
        assert "min_2_day_window_pressure" in result.schema.names
        assert "sum_4_day_window_wind_speed" in result.schema.names
    ...
    def test_calculate_feature_invalid_time_column(self, sample_time_table: pa.Table) -> None:
        ...
        with pytest.raises(
            ValueError,
            match=f"Time filter feature '{DefaultOptionKeys.reference_time}' must be a timestamp column",
        ):
            PyArrowTimeWindowFeatureGroup.calculate_feature(table, feature_set)
